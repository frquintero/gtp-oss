...
class Config:
    ...
    def __init__(self):
        self.settings = {
            ...
            'thinking_mode': os.getenv('GPT_THINKING_MODE', 'normal'),
        }
        
        # Valid options
        self.valid_reasoning_efforts = ['low', 'medium', 'high']
        self.valid_thinking_modes = ['normal', 'hard']
    ...
    def set(self, key: str, value: Any) -> None:
        ...
        if key == 'thinking_mode' and value not in self.valid_thinking_modes:
            raise ValueError(f"Invalid thinking mode '{value}'. Valid options: {', '.join(self.valid_thinking_modes)}")
    ...
    def save_to_file(self, filepath: str) -> None:
        ...
        # If the file already exists, preserve its structure
        try:
            with open(filepath, 'r') as f:
                existing_config = json.load(f)
                
            # Update the existing structure instead of replacing it
            existing_config.update(save_data)
            
            # Preserve the ui section if it exists
            if 'ui' in existing_config and isinstance(existing_config['ui'], dict):
                save_data = existing_config
    ...
    def toggle_thinking_mode(self) -> str:
        """Toggle between 'normal' and 'hard' thinking modes."""
        current_mode = self.get('thinking_mode', 'normal')
