...
class CommandPalette:
    ...
    def _register_default_commands(self):
        ...
        self.add_command("thinking", "Toggle between normal and hard thinking modes", 
                        lambda: self._execute_thinking_toggle(), "Quick Actions")
...
Type [cyan]help[/cyan] for available commands."""
    ...
    def _execute_thinking_toggle(self):
        """Execute thinking mode toggle and refresh UI."""
        # Debug output
        current_mode = self.cli.config.get('thinking_mode', 'normal')
        self.console.print(f"[dim]Palette Debug: Current mode before toggle: {current_mode}[/dim]")
        
        # Execute the thinking command
        result = self.cli.handle_command("thinking")
        
        # Debug output after toggle
        new_mode = self.cli.config.get('thinking_mode', 'normal')
        self.console.print(f"[dim]Palette Debug: Mode after toggle: {new_mode}[/dim]")
        
        # Clear the screen and redisplay welcome/status
        try:
            self.cli.console.clear()
        except:
            pass
            
        # Force refresh of welcome and status display
        if hasattr(self.cli, 'display_welcome'):
            self.cli.display_welcome()
        if hasattr(self.cli, 'display_status_bar'):
            self.cli.display_status_bar()
